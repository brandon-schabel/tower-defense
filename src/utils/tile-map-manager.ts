...
export default class TileMapManager {
    ...
    public occupyTiles(tileX: number, tileY: number, width: number, height: number, type: string, id: string): void {
        for (let y = tileY; y < tileY + height; y++) {
            for (let x = tileX; x < tileX + width; x++) {
                const key = `${x},${y}`;
                this.occupancyMap.set(key, { type, id });
            }
        }
    }
    ...
    public findValidPlacement(width: number, height: number): { tileX: number, tileY: number } | null {
        // Try to find a valid placement by checking random positions
        const maxAttempts = 100;
        
        for (let i = 0; i < maxAttempts; i++) {
            const x = Phaser.Math.RND.between(0, this.mapWidth - width);
            const y = Phaser.Math.RND.between(0, this.mapHeight - height);
            
            if (this.isTileAvailable(x, y, width, height)) {
                return { tileX: x, tileY: y };
            }
        }
        
        // Fallback to a more systematic approach if random attempts fail
        for (let y = 0; y < this.mapHeight - height; y++) {
            for (let x = 0; x < this.mapWidth - width; x++) {
                if (this.isTileAvailable(x, y, width, height)) {
                    return { tileX: x, tileY: y };
                }
            }
