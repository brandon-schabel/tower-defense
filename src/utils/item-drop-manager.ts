...
import { ALL_ITEMS, ITEM_DROP_CONFIG } from '../data/items';
...
export default class ItemDropManager {
    ...
    private pickupRange: number = 50; // Pixels radius for pickup
    ...
    public dropRandomItem(x: number, y: number): DroppedItem {
        // Determine which item to drop based on probabilities
        const roll = Math.random();
        let item: GameItem;
        
        if (roll < 0.5) {
            // 50% chance of resources
            item = this.createResourceItem();
        } else if (roll < 0.8) {
            // 30% chance of health
            item = this.createHealthItem();
        } else {
            // 20% chance of weapon
            item = this.createWeaponItem();
        }
        
        return this.dropItem(item, x, y);
    }
